...
g_dictEmptyLineDescriptor['left_text'] = str("")
g_dictEmptyLineDescriptor['right_text'] = str("")
...
class LcdBase():
	def __init__(self):
		...
		self.m_reLeftText = None
		self.m_reRightText = None
...
# @abstractmethod
	...
	def Render(self, mode, bForce):
		...
		if (self.m_reLeftText == None):
			self.m_reLeftText = re.compile(r"\$LEFT\[(.+?)]" , re.IGNORECASE)
		if (self.m_reRightText == None):
			self.m_reRightText = re.compile(r"\$RIGHT\[(.+?)]" , re.IGNORECASE)
		...
		while (outLine < int(self.GetRows()) and inLine < len(self.m_lcdMode[mode])):
			...
			if self.m_lcdMode[mode][inLine]['type'] == LCD_LINETYPE.LCD_LINETYPE_PROGRESS:
				...
				leftmatches = self.m_reLeftText.findall(srcline)
				srcline = self.m_reLeftText.sub('', srcline)
				leftline = ""
				for match in leftmatches:
					leftline += match
					
				rightmatches = self.m_reRightText.findall(srcline)
				srcline = self.m_reRightText.sub('', srcline)
				rightline = ""
				...
				if len(leftline) > 0:
					leftline = self.StripBBCode(leftline)
				
				if len(rightline) > 0:
					rightline = self.StripBBCode(rightline)
				...
				if self.m_strInfoLabelEncoding != self.m_strLCDEncoding:
					...
					try:
						leftline = leftline.decode(self.m_strInfoLabelEncoding).encode(self.m_strLCDEncoding, "replace")
					except:
						log(xbmc.LOGDEBUG, "Caught exception on charset conversion: " + leftline)
						leftline = ""
					
					try:
						rightline = rightline.decode(self.m_strInfoLabelEncoding).encode(self.m_strLCDEncoding, "replace")
					except:
						log(xbmc.LOGDEBUG, "Caught exception on charset conversion: " + rightline)
						rightline = "---"
				else:
					line = srcline
